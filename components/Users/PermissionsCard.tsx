...
export function PermissionsCard({ permissions, setPermissions }: Props) {
  const [allPermissions, setAllPermissions] = useState<Permission[]>([]);

  useEffect(() => {
    apiService.getAllPermissions().then((res) => {
      setAllPermissions(res);

      const initialChecked: { [key: string]: boolean } = {};
      res.forEach((perm) => {
        if (!(perm.name in permissions)) initialChecked[perm.name] = false;
        else initialChecked[perm.name] = permissions[perm.name];
      });

      setPermissions(initialChecked);
    });
  }, []);
